...
/* ========================================================================== */
void fn_dirFDaubechiesWT2p2
(
  float                    *a_ka,                                               /* pointer to array with coefficients                */
  ...
  switch (cuic_w)
    ...
    case 1:
      for (ui_j=0; ui_j<cuci_N; ui_j=ui_j+2)
        {
          a_ka[(ui_j>>1)]             = (a_da[ui_j] - a_da[ui_j+1])*HK;         /* High pass */
          a_ka[(ui_j>>1)+(cuci_N>>1)] = (a_da[ui_j] + a_da[ui_j+1])*HK;         /* Low pass  */
        }
      break;
    ...
    default:
      ...
      for (ui_j=0; ui_j<cuci_N; ui_j=ui_j+2)
        {
          a_ka[ui_j>>1]    = (a_da[ui_j] - a_da[ui_j+1])*HK;                    /* High pass */
          a_buff[ui_j>>1] = (a_da[ui_j] + a_da[ui_j+1])*HK;                     /* Low pass  */
        }
      ...
      for (ui_i=1; ui_i<(cuic_w-1); ui_i++)                                     /* deconstruction levels of FWT - without last and first */
        {
          ...
          for (ui_j=0; ui_j<ui_ord; ui_j=ui_j+2)
            {
              a_ka[ui_step+(ui_j>>1)] = (a_buff[ui_j] - a_buff[ui_j+1])*HK;     /* High pass */
              a_buff[ui_j>>1]        = (a_buff[ui_j] + a_buff[ui_j+1])*HK;      /* Low pass  */
            }
        }
      ...
      for (ui_j=0; ui_j<ui_ord; ui_j=ui_j+2)
        {
          a_ka[ui_step+(ui_j>>1)] = (a_buff[ui_j] + a_buff[ui_j+1])*HK;         /* Low pass  */
        }
      break;
    }
  return;
...
/* -------------------------------------------------------------------------- */
{
  ...
  switch (cuic_w)
    ...
    case 1:
      for (ui_j=0; ui_j<cuci_N; ui_j=ui_j+2)
        {
          a_da[ui_j]   = (a_ka[(ui_j>>1)+(cuci_N>>1)])*HK;                      /* Low pass  */
          a_da[ui_j+1] = (a_ka[(ui_j>>1)+(cuci_N>>1)])*HK;                      /* Low pass  */

          a_da[ui_j]   = a_da[ui_j]   + (a_ka[(ui_j>>1)])*HK;                   /* High pass */
          a_da[ui_j+1] = a_da[ui_j+1] - (a_ka[(ui_j>>1)])*HK;                   /* High pass */
        }
      break;
    ...
    default:
      ...
      for (ui_j=0; ui_j<(ui_ord<<1); ui_j=ui_j+2)
        {
          a_buff[ui_j]   = (a_ka[(ui_j>>1)+ui_step])*HK;                        /* Low pass  */
          a_buff[ui_j+1] = (a_ka[(ui_j>>1)+ui_step])*HK;                        /* Low pass  */
        }
      ...
      for (ui_j=0; ui_j<ui_ord; ui_j=ui_j+2)
        {
          a_buff[ui_j]   = a_buff[ui_j]   + (a_ka[(ui_j>>1)+ui_step])*HK;       /* High pass */
          a_buff[ui_j+1] = a_buff[ui_j+1] - (a_ka[(ui_j>>1)+ui_step])*HK;       /* High pass */
        }
          ...
          for (ui_j=0; ui_j<ui_ord; ui_j=ui_j+2)
            {
              ...
              p_p2[ui_j]   = p_p2[ui_j]   + (a_ka[(ui_j>>1)+ui_step])*HK;       /* High pass */
              p_p2[ui_j+1] = p_p2[ui_j+1] - (a_ka[(ui_j>>1)+ui_step])*HK;       /* High pass */
            }
          /* swapping buff roles  */
          p_temp = p_p1;
          p_p1   = p_p2;
...
/* ========================================================================== */
void fn_dirFDaubechiesWT2p2oneStage
(
    float                    *a_ka,                                             /* pointer to array with coefficients                */
    float                    *a_da,                                             /* pointer to array with samples                     */
    const unsigned int const  cuci_N                                            /* number elements in array                          */
...
/* ========================================================================== */
void fn_invFDaubechiesWT2p2oneStage
(
    float                    *a_da,                                             /* pointer to array with samples                     */
    float                    *a_ka,                                             /* pointer to array with coefficients                */
    const unsigned int const  cuci_N                                            /* number elements in array                          */
