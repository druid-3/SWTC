...
(
    int                       *ai_ka[],                                         /* pointer to arrays with coefficients               */
    ...
    switch (cuic_w)
        ...
        case 1:
            for (ui_j=0; ui_j<cuci_N; ui_j=ui_j+2)
                {
                    *(ai_ka[0]+(ui_j>>1)) = (ai_da[ui_j] - ai_da[ui_j+1])>>1;                  /* High pass */
                    *(ai_ka[1]+(ui_j>>1)) = (ai_da[ui_j] + ai_da[ui_j+1])>>1;                  /* Low pass  */
                }
            break;
        ...
        default:
            ...
            for (ui_i=1; ui_i<(cuic_w-1); ui_i++)                                               /* deconstruction levels of FWT - without last and first */
                {
                    ...
                    for (ui_j=0; ui_j<ui_ord; ui_j=ui_j+2)
                        {
                            *(ai_ka[ui_i]+(ui_j>>1)) = (ai_buff[ui_j] - ai_buff[ui_j+1])>>1;    /* High pass */
            ...
            for (ui_j=0; ui_j<ui_ord; ui_j=ui_j+2)
                {
                    *(ai_ka[ui_i]+(ui_j>>1)) = (ai_buff[ui_j] + ai_buff[ui_j+1])>>1;            /* Low pass  */
                }
            break;
        }
    return;
...
/* -------------------------------------------------------------------------- */
{
    ...
    switch (cuic_w)
        ...
        case 1:
            for (ui_j=0; ui_j<cuci_N; ui_j=ui_j+2)
                {
                    ai_da[ui_j]   = *(ai_ka[1]+(ui_j>>1));                      /* Low pass  */
                    ai_da[ui_j+1] = *(ai_ka[1]+(ui_j>>1));                      /* Low pass  */

                    ai_da[ui_j]   += *(ai_ka[0]+(ui_j>>1));                     /* High pass */
                    ai_da[ui_j+1] -= *(ai_ka[0]+(ui_j>>1));                     /* High pass */
                }
            break;
        ...
        default:
            ...
            for (ui_j=0; ui_j<ui_ord; ui_j=ui_j+2)
                {
                    ai_buff[ui_j]   = *(ai_ka[cuic_w]+(ui_j>>1));                    /* Low pass  */
                    ai_buff[ui_j+1] = *(ai_ka[cuic_w]+(ui_j>>1));                    /* Low pass  */

                    ai_buff[ui_j]   += *(ai_ka[cuic_w-1]+(ui_j>>1));                 /* High pass */
                    ai_buff[ui_j+1] -= *(ai_ka[cuic_w-1]+(ui_j>>1));                 /* High pass */
                }
                    ...
                    for (ui_j=0; ui_j<ui_ord; ui_j=ui_j+2)
                        {
                            ...
                            pi_p2[ui_j]   += *(ai_ka[cuic_w-ui_i-1]+(ui_j>>1));      /* High pass */
                            pi_p2[ui_j+1] -= *(ai_ka[cuic_w-ui_i-1]+(ui_j>>1));      /* High pass */
                        }
                    /* swapping buff roles  */
                    pi_temp = pi_p1;
                    pi_p1   = pi_p2;
            ...
            for (ui_j=0; ui_j<cuci_N; ui_j=ui_j+2)
                {
                    ...
                    ai_da[ui_j]   += *(ai_ka[cuic_w-ui_i-1]+(ui_j>>1));              /* High pass */
                    ai_da[ui_j+1] -= *(ai_ka[cuic_w-ui_i-1]+(ui_j>>1));              /* High pass */
                }
            break;
        }
    return;
...
(
    int                      *ai_kaHF,                                          /* pointer to array with coefficients HF             */
    int                      *ai_kaLF,                                          /* pointer to array with coefficients LF             */
    int                      *ai_da,                                            /* pointer to array with samples                     */
    const unsigned int const  cuci_N                                            /* number elements in array                          */
)
/* -------------------------------------------------------------------------- */
{
    ...
    for (ui_j=0; ui_j<cuci_N; ui_j=ui_j+2)
        {
            ai_kaHF[ui_j>>1] = (ai_da[ui_j] - ai_da[ui_j+1])>>1;                /* High pass */
            ai_kaLF[ui_j>>1] = (ai_da[ui_j] + ai_da[ui_j+1])>>1;                /* Low pass  */
        }
    return;
}
/* ========================================================================== */
...
/* -------------------------------------------------------------------------- */
{
    ...
    for (ui_j=0; ui_j<cuci_N; ui_j=ui_j+2)
        {
            ai_da[ui_j]    = ai_kaLF[ui_j>>1];                                  /* Low pass  */
            ai_da[ui_j+1]  = ai_kaLF[ui_j>>1];                                  /* Low pass  */

            ai_da[ui_j]   += ai_kaHF[ui_j>>1];                                  /* High pass */
            ai_da[ui_j+1] -= ai_kaHF[ui_j>>1];                                  /* High pass */
        }
    return;
}
/* --------------------------------------------------------------------------
